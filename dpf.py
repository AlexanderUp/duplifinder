...
class Duplifinder:
        ...
        if self._extensions:
            ...
            for root, _, files in os.walk(self._path):
                for data_file in files:
                    ...
                    is_path_to_file_in_db: FileHash | None = self.session.query(FileHash).filter(FileHash.path == path).first()
    ...
    def duplicate_hash_query_generator(self) -> Generator[list, None, None]:
        ...
        duplicate_hash_entities: list[tuple[str]] = (
            ...
            .having(func.count(FileHash.file_hash) > 1)  # type: ignore
            .all()
        )
        ...
        for duplicate_hash in duplicate_hashes:
            query: list[FileHash] = self.session.query(FileHash).filter(FileHash.file_hash == duplicate_hash).all()
    ...
    def _clean_up_db(self) -> None:
        ...
        for db_file in files:
            if not os.path.exists(db_file.path) and not db_file.is_deleted:
                logger.info(
                    "File does not exist! Marking as has been deleted...\t{db_file.path}",
                )
...
if __name__ == "__main__":
    ...
    type_group.add_argument(
        ...
        "--documents",
        action="store_true",
        help=("Update database with hashes of document and " "non-media files in given directory"),
    )
